...
function AddKitRow() {
	...
	if(valid)
	{
		...
		var revNos = $("input[name='pfc_rev_no_kit[]']").map(function(){return $(this).val();}).get();
		if($.inArray($("#kit_item_id").val(),ids) >= 0 && $.inArray($("#process_id").val(),processIds) >= 0  && $.inArray($("#pfc_rev_no_kit").val(),revNos) >= 0){
            ...
            cell = $(row.insertCell(-1));
			cell.html($("#pfc_rev_no_kit").val() + '<input type="hidden" name="pfc_rev_no_kit[]" value="'+$("#pfc_rev_no_kit").val()+'">');
