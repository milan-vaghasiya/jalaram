$(document).ready(function(){
	...
	$(document).on('click','.createSalesOrder',function(){
		var party_id = $(this).data('customer_id');
		...
		if(party_id != "" || party_id != 0){
			$.ajax({
				url : base_url + '/salesQuotation/getPartyOrders',
				type: 'post',
				data:{party_id:party_id},
				dataType:'json',
				success:function(data){
					$("#SalesorderModal").modal();
					$("#exampleModalLabel1").html('Create Sales Order');
					$("#party_salesOrder").attr('action',base_url + 'salesOrder/createOrder');
					$("#btn-create").html('<i class="fa fa-check"></i> Create Order');
					$("#partyName").html(party_name);
					$("#party_name_so").val(party_name);
					$("#party_id_so").val(party_id);
					$("#qo_id_so").val(quote_id);
					$("#salesOrderData").html("");
					$("#salesOrderData").html(data.htmlData);
					//orderTable();
				}
			});
		} else {
