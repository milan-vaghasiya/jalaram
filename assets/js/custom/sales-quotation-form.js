...
// 			$("#contact_email").val(partyData.contact_email);
			if(isNaN(partyData.party_phone)){ $("#party_phone").val(partyData.party_phone); }
			if(isNaN(partyData.party_email)){ $("#party_email").val(partyData.party_email); }
			if(isNaN(partyData.party_address)){ $("#party_address").val(partyData.party_address); }
			if(isNaN(partyData.party_pincode)){ $("#party_pincode").val(partyData.party_pincode); }
			if(isNaN(partyData.sales_executive)){ $("#sales_executive").val(partyData.sales_executive); }
			if(isNaN(partyData.party_name)){ $("#party_name").val(partyData.party_name); }
			...
			$.ajax({				
				url: base_url + 'parties/getPartyContactDetail',
				...
				success: function (data) {
					...
					$("#contact_person").html(data.person);
					$("#contact_no").html(data.mobile);
					$("#contact_email").html(data.email);

					$("#contact_person").comboSelect();
					$("#contact_no").comboSelect();
					$("#contact_email").comboSelect();
				}
			});
		}else{
// 			$("#contact_person").val("");
    ...
    $(document).on('click','.saveItem',function(){
        ...
        if(formData.item_name == ""){
			...
			if ($.inArray(formData.item_name,item_names) >= 0 && formData.row_index == "") {
				...
				if(formData.qty == "" || formData.qty == "0" || formData.price == "" || formData.price == "0" || formData.unit_id == ""){
                    ...
                    if($(this).data('fn') == "save"){
                        ...
                        $("#item_id").comboSelect();
                        $("#unit_id").comboSelect();
						$("#row_index").val($('#salesEnqItems tbody').find('tr').length);
...
function AddRow(data) {
	...
	var moqInput = $("<input/>",{type:"hidden",name:"batch_qty[]",value:data.batch_qty});
	var gradeInput = $("<input/>",{type:"hidden",name:"close_reason[]",value:data.close_reason});
	var samplechargeInput = $("<input/>",{type:"hidden",name:"packing_qty[]",value:data.packing_qty});
	cell = $(row.insertCell(-1));
	cell.html(data.item_remark);
	cell.append(itemRemarkInput);
	cell.append(prodDisInput);
	cell.append(moqInput);
	cell.append(gradeInput);
	cell.append(samplechargeInput);
