$(document).ready(function () {
		...
		if (party_id != "" || party_id != 0) {
			$.ajax({
				...
				success: function (data) {
					$("#orderModal").modal();
					$("#orderModal #createInvoice").html('Create Invoice');
					$("#orderModal #party_so").attr('action', base_url + 'purchaseInvoice/createInvoice');
					$("#orderModal #btn-create").html('<i class="fa fa-check"></i> Create Invoice');
					$("#orderModal #partyName").html(party_name);
					$("#orderModal .modal-title").html("GRN ["+party_name+"]");
					$("#orderModal #party_name_pinv").val(party_name);
					$("#orderModal #party_id_pinv").val(party_id);
	...
	$(document).on('click', '.createQcPurchaseInvoice', function () {
		var party_id = $('#party_id').val();
		var party_name = $('#party_idc').val();
		$('.party_id').html("");
		...
		if (party_id != "" || party_id != 0) {
			$.ajax({
				url: base_url + 'purchaseInvoice/getPartyQcPurchase',
				type: 'post',
				data: { party_id: party_id },
				dataType: 'json',
				success: function (data) {
					$("#qcPurchaseModal").modal();
					$("#qcPurchaseModal #createInvoice").html('Create Invoice');
					$("#qcPurchaseModal #party_qc").attr('action', base_url + 'purchaseInvoice/createQcPurchaseInvoice');
					$("#qcPurchaseModal #btn-create").html('<i class="fa fa-check"></i> Create Invoice');
					$("#qcPurchaseModal #partyName").html(party_name);
					$("#qcPurchaseModal .modal-title").html("QC PO ["+party_name+"]");
					$("#qcPurchaseModal #party_name_pinv").val(party_name);
					$("#qcPurchaseModal #party_id_pinv").val(party_id);
					
					$("#qcPurchaseModal #qcData").html("");
					$("#qcPurchaseModal #qcData").html(data.htmlData);
				}
			});
		} else {
			$('.party_id').html("Party is required.");
	...
	$(document).on('click','.qcPurcOrd',function(){
		if($(this).prop('checked') == true){
			var order_id = $(this).data('order_id');
			$(".order_id_"+order_id).prop('checked',true);
		}
	});
	...
	$(document).on('click', '.add-item', function () {
		...
		if (party_id) {
			$.ajax({
				type: "POST",
				url: base_url + controller + '/getGrnItem',
				data: { party_id: party_id },
				dataType: 'json',
