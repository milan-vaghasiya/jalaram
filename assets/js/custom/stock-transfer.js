...
function itemTransfer(data){
    var button = "";
	var fnSave = data.fnSave;if(fnSave == "" || fnSave == null){fnSave="save";}

    $.ajax({ 
		type: "POST",   
		url: base_url + controller + '/itemTransfer',   
		data: {location_id:data.location_id,item_id:data.item_id,stock_qty:data.stock_qty,batch_no:data.batch_no}
	}).done(function(response){
		$("#"+data.modal_id).modal();
		$("#"+data.modal_id+' .modal-title').html(data.title);
		$("#"+data.modal_id+' .modal-body').html(response);
		$("#"+data.modal_id+" .modal-body form").attr('id',data.form_id);
		$("#"+data.modal_id+" .modal-footer .btn-save").attr('onclick',"saveItemTrans('"+data.form_id+"','"+fnSave+"');");
		if(button == "close"){
			$("#"+data.modal_id+" .modal-footer .btn-close").show();
			$("#"+data.modal_id+" .modal-footer .btn-save").hide();
		}else if(button == "save"){
...
function saveItemTrans(formId,fnSave){
    setPlaceHolder();
	if(fnSave == "" || fnSave == null){fnSave="save";}
	var form = $('#'+formId)[0];
	var fd = new FormData(form);
