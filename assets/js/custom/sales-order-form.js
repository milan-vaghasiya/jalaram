$(document).ready(function(){
    ...
    $(document).on('change','#item_id',function(){
		if($(this).val() == ""){
			...
			$("#item_alias").val(itemData.item_alias);
			...
			if(order_id){
				$.ajax({ 
					type: "POST",   
					url: base_url + controller + '/getSOEstimateQty',   
	...
	$(document).on("change keyup","#party_id",function(){
		...
		if($(this).val() != ""){
			...
			var gstin = partyData.gstin;
			var currency = partyData.currency;
			$("#currency").val(currency);
			$("#currency").comboSelect();
...
// 			}else {
				...
				if(((formData.qty == "" || formData.qty == "0") && (formData.qty_kg == "" || formData.qty_kg == "0")) || (formData.price == "" || formData.price == "0" || formData.price == "0.000")){
					if((formData.qty == "" || formData.qty == "0") && (formData.qty_kg == "" || formData.qty_kg == "0")){
						$(".qty").html("Order Qty or Estimate Qty is required.");
					}
					if(formData.price == "" || formData.price == "0" || formData.price == "0.000"){
						$(".price").html("Price is required.");
					}
					if(sales_type == 2 && formData.item_alias == ""){
						$('.item_alias').html('Product alias is required.');
					}
					/* if(sales_type == 2 && formData.org_price == "" || sales_type == 2 && formData.org_price == "0"){
						$(".org_price").html("Price is required.");
					} */
				}else{
...
function AddRow(data) {
	...
	var itemAliasInput = $("<input/>",{type:"hidden",name:"item_alias[]",value:data.item_alias});	
	...
	var protargetDateInput = $("<input/>",{type:"hidden",name:"prod_target_date[]",value:data.prod_target_date});
	...
	cell.append(protargetDateInput);
	...
	var scheduleType = $("<input/>",{type:"hidden",name:"schedule_type[]",value:data.schedule_type});
    cell = $(row.insertCell(-1));
	cell.html(data.hsn_code );
	cell.append(hsnCodeInput);
	cell.append(ppapLevel);
	cell.append(scheduleType);
