...
function AddRow(data) {
	...
	var machineInput = $("<input/>",{type:"hidden",name:"machine_id[]",value:data.machine_id});	
	...
	var itmname = data.item_name;
	if(data.item_code !== ""){itmname = '[' + data.item_code + '] ' + data.item_name;}
	cell = $(row.insertCell(-1));
	cell.html(itmname);
	cell.append(itemIdInput);
	cell.append(itemNameInput);
	cell.append(transIdInput);
	cell.append(machineInput);
	cell.append(locationIdInput);
	// cell.append(batchNoInput);
	cell.append(locationErrorDiv);
	// cell.append(batchNoErrorDiv);
	
	var collectedbyInput = $("<input/>",{type:"hidden",name:"collected_by[]",value:data.collected_by});
	var collectedbyErrorDiv = $("<div></div>",{class:"error collected_by"+countRow});
	cell = $(row.insertCell(-1));
	cell.html(data.collected_by);
	cell.append(collectedbyInput);
	cell.append(collectedbyErrorDiv);
	
	var returnableText = '';
	if(data.is_returnable == '1' || data.is_returnable == 1){returnableText = 'Yes';}else{returnableText = 'No';}
	var returnableInput = $("<input/>",{type:"hidden",name:"is_returnable[]",value:data.is_returnable});
	var returnableErrorDiv = $("<div></div>",{class:"error is_returnable"+countRow});
	cell = $(row.insertCell(-1));
	cell.html(returnableText);
	cell.append(returnableInput);
	cell.append(returnableErrorDiv);
