...
class AutoCron extends CI_Controller{
    ...
    public function updateShiftlogV1(){
		...
		$this->db->select("employee_master.id,employee_master.shift_id,shift_master.latest_id");
		...
		//$day = 5;$cmonth = 1;$cyear = 2023;$inserted=0;$updated=0;
		$day = date('d');$cmonth = date('m');$cyear = date('Y');$inserted=0;$updated=0; //$day = 1;
        if(!empty($empData)):
	...
	/*** Created By JP @09-02-2024 https://jalaram.scubeerp.in/autoCron/updateShiftlogManual ***/
    public function updateShiftlogManual(){
        
		$this->db->select("id,d8");
        $this->db->where('month','2024-02-01');
		$empShiftData = $this->db->get('emp_shiftlog')->result();
		
		$updated=0;
        if(!empty($empShiftData)):
			foreach($empShiftData as $row):
				$empShiftLog = Array();
				for($i=9;$i<=29;$i++)
				{
					$field = 'd'.$i;
					$empShiftLog[$field] = $row->d8;
				}
				
			    $this->db->reset_query();
				$this->db->where('id',$row->id);
				$this->db->update('emp_shiftlog',$empShiftLog);
				$updated++;
			endforeach;	
        endif;
	...
	public function getIdleReasonBySrno(){
	    $postData = $this->input->post();
		$this->db->select('rejection_comment.sr_no,rejection_comment.code,rejection_comment.remark as reason');
        $this->db->where('type',2);
        $this->db->where('is_delete',0);
        if(!empty($postData['sr_no']))
        {
            $this->db->where('sr_no',$postData['sr_no']);
            $result = $this->db->get('rejection_comment')->row();
        }
        else
        {
