...
class PurchaseInvoice extends MY_Controller
{
	...
	public function createInvoice(){
		...
		$orderItems = $this->purchaseInvoice->getGrnItemsForInvoice($data['ref_id']);
	...
	public function getPartyQcPurchase(){
		$orderItems = $this->qcPurchase->getQcPurchaseItemsForInvoice(['party_id'=>$this->input->post('party_id')]);
		$html="";
        if(!empty($orderItems)):
            $i=1;
            foreach($orderItems as $row):
                $html .= '<tr>
                    <td class="text-center">
                        <input type="checkbox" id="md_checkbox_'.$i.'" name="ref_id[]" class="filled-in chk-col-success qcPurcOrd order_id_'.$row->order_id.'" data-order_id="'.$row->order_id.'" value="'.$row->id.'" ><label for="md_checkbox_'.$i.'" class="mr-3 check'.$row->id.'"></label>
                    </td>
                    ...
                    <td class="text-center">'.floatval(($row->rec_qty - $row->inv_qty)).'</td>
                </tr>'; $i++;
            endforeach;
        else:
		...
		$this->printJson(['status'=>1,'htmlData'=>$html,'result'=>$orderItems]);
	}

	public function createQcPurchaseInvoice(){
		$data = $this->input->post(); 
		$orderItems = $this->qcPurchase->getQcPurchaseItemsForInvoice(['ref_ids'=>$data['ref_id']]);
		$orderData = new stdClass();
		$orderData->party_id = $data['party_id'];		
		$orderData->id = implode(",",array_unique(array_column($orderItems,'order_id')));
		$this->data['qcOrderItems'] = $orderItems;
		$this->data['orderData'] = $orderData;
		$this->data['ref_id']= $orderData->id;
		$this->data['party_id'] = $data['party_id'];
		$this->data['party_name'] = $data['party_name'];
