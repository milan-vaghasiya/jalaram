...
class Report extends MY_Controller
{
    ...
    public function supplierPurchase(){
		$this->data['headData']->pageTitle = "Approve Supplier";
        $this->data['pageHeader'] = 'LIST OF APPROVED SUPPLIER (PURCHASE/SERVICE) - Doc.No. D PU 01(Rev.No. 00/01.06.20)';
    ...
    public function getSupplierPurchase(){        
        ...
        foreach($result['data'] as $row):
            ...
            $sendData[] = [$i++,$row->party_code,$row->party_name."<br><small>".$row->party_address."</small>",$row->contact_person,$row->party_phone,'',$row->party_email,$row->party_mobile,$row->supplied_types,$row->approved_date,$row->approved_by,$row->approved_base,''];
        endforeach;
        $result['data'] = $sendData;
        $this->printJson($result);
    }
