...
class DebitNote extends MY_Controller
{
	...
	public function getDTRows(){
		...
		foreach ($result['data'] as $row) :
			$doc_no = (!empty($row->doc_no))?explode(',',$row->doc_no)[0]:'';
		    $invData = $this->debitNote->getPinvFromInvno($doc_no,$row->party_id);
		    $row->doc_date = (!empty($invData->trans_date)?formatDate($invData->trans_date) : '');
	...
	public function getItemLocationWiseBatch(){
		...
		$batchData = $this->debitNote->getItemLocationWiseBatch($item_id,$location_id);//,$party_id);

		$options = '<option value="" data-batch_qty="0">Select Batch No.</option>';
		foreach($batchData as $row):
			if(!empty($batch_no) && $batch_no == $row->batch_no || $row->batch_qty > 0 ):
				$options .= '<option value="'.$row->batch_no.'" data-stock_qty="'.$row->batch_qty.'">'.$row->batch_no.' | BQ.:'.$row->batch_qty.'</option>';
			endif;
		endforeach;
		$this->printJson(['status'=>1,'options'=>$options]);
	}
	...
	public function debitNote_pdf($id){
		$this->data['dnData'] = $dnData = $this->debitNote->getDebitNote($id);
		$doc_no = (!empty($dnData->doc_no))?explode(',',$dnData->doc_no)[0]:'';
		$this->data['invData'] = $invData = $this->debitNote->getPinvFromInvno($doc_no,$dnData->party_id);
		$this->data['partyData'] = $this->party->getParty($this->data['dnData']->party_id);
		$this->data['companyData'] = $companyData = $this->debitNote->getCompanyInfo();
 
		$response="";
		$logo=base_url('assets/images/logo.png'); $unapproved = base_url('assets/images/unapproved.jpg');
		$this->data['letter_head']=base_url('assets/images/letterhead_top.png');

		$pdfData = $this->load->view('debit_note/print_debit_note',$this->data,true);
		...
		<table class="table top-table" style="margin-top:10px;">
			<tr>
				<td style="width:25%;">DN No. & Date : '.$dnData->trans_prefix.$dnData->trans_no.' '.formatDate($dnData->trans_date).'</td>
