                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table id='iotCongigTble' class="table table-bordered ssTable" data-url='/getDTRows'></table>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        $(document).on('click', ".addIdleTime", function() {
            var id = $(this).data('id');
            console.log(id);
            var machine_code = $(this).data('machine_code');
            var functionName = $(this).data("function");
            var modalId = $(this).data('modal_id');
            var button = $(this).data('button');
            var title = $(this).data('form_title');
            var formId = functionName;
			var fnsave = 'saveIdleTime';
			var srposition = 0;
            ...
            $.ajax({
                ...
                $("#" + modalId).modal();
                $("#" + modalId + ' .modal-title').html(title + " [ Machine : "+machine_code+" ]");
