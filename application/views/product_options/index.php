                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <select name="is_child" id="is_child" class="form-control float-right" style="width:40%">
...
<script>
    $(document).ready(function() {
        ...
        $(document).on("change","#is_child",function(){
            $("#productOptionTable").attr("data-url",'/getDTRows/'+$(this).val());
            ssTable.state.clear();initTable(0);
        });
    });
    function pfcRevProcess(pfc_rev_no,item_id){
       
        $.ajax({ 
            type: "POST",   
            url: base_url + controller + '/pfcRevProcess',   
            data: {pfc_rev_no:pfc_rev_no,item_id:item_id}
        }).done(function(response){
            $("#modal-lg").modal();
            $("#modal-lg").css('z-index','9999')
			$("#modal-lg"+' .modal-title').html("PRocess List : "+pfc_rev_no);
