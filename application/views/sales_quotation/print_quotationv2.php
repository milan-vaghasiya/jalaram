	...
	<div class="col-12">
		...
		<table class="table item-list-bb" style="margin-top:25px;">
			<tr>
				...
				<th>MOQ.</th>
				<th>RM Grade</th>
				<th style="width:100px;">EAU</th>
				<th style="width:50px;">UOM</th>
				<th style="width:60px;">Rate<br><small>(<?=$sqData->lr_no?>)</small></th>
				<th style="width:60px;">Sample Charge<br><small>(<?=$sqData->lr_no?>)</small></th>
				<?= $discCol ?>
				...
				if(!empty($sqData->itemData)):
					foreach($sqData->itemData as $row):
						...
						echo '<tr>';
							...
							echo '<td class="text-center" style="padding-right:8px;">'.sprintf('%.3f',$row->price).'</td>';
							echo '<td class="text-center" style="padding-right:8px;">'.$row->packing_qty.'</td>';
							echo $discColVal;
