                ...
                <div class="dz-tab style-4">
    				...
    				<div class="tab-content px-0 py-0 list-grid" id="myTabContent1"  data-isotope='{ "itemSelector": ".listItem" }'>
						<ul class="dz-list message-list" >
							<?=!empty($salesOrdList)?$salesOrdList:''?>
						</ul>
    				</div>
			    </div>
			</div>    
...
$(document).ready(function(){
	...
	$(document).on( 'click', '.buttonFilter', function() {
		...
		$(".message-list").html("");	
		$.ajax({
		url: base_url  + 'app/salesOrder/getSalesOrderData',
		data:{'status':status},
		type: "POST",
		dataType:"json",
	...
	$(document).on('click','.approveSO',function(){
		...
		Swal.fire({
			title: 'Confirm!',
			text: "Are you sure want to Approve this Sales Order ?",
			// icon: 'warning',
			...
			{
				$.ajax({
					url: base_url + 'app/salesOrder/approveSOrder',
					data: send_data,
					type: "POST",
					dataType:"json",
				}).done(function(response){
...
function responseFunction(response){
    if(response.status==0){
		...
		Swal.fire({
            ...
            window.location = base_url + 'app/salesOrder';
        });
