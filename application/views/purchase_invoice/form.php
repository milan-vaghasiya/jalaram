					...
					<div class="card-body">
						<form autocomplete="off" id="savePurchaseInvoice">
							<div class="col-md-12">
								...
								<input type="hidden" name="reference_entry_type" id="reference_entry_type" value="<?= (!empty($invoiceData->from_entry_type)) ? $invoiceData->from_entry_type : ((!empty($from_entry_type))?$from_entry_type:"") ?>">
								...
								<div class="row form-group">
									...
									<div class="col-md-2">
										...
										<input type="date" id="inv_date" name="inv_date" class=" form-control req inv_date" placeholder="dd-mm-yyyy" value="<?= (!empty($invoiceData->trans_date)) ? $invoiceData->trans_date : $maxDate ?>" min="<?=$startYearDate?>" max="<?=$maxDate?>" />
									</div>
									<div class="col-md-3">
									...
									<div class="col-md-5">
										...
										<div for="party_id1" class="ml-3 float-right">
											<a href="javascript:void(0)" class="text-primary font-bold createQcPurchaseInvoice permission-write1" datatip="QC PO." flow="down">+ QC PO.</a>
										</div>
					...
					<div class="col-md-12">
						<div class="row form-group">
							...
							<div class="col-md-12 form-group">
								...
								<select name="item_id" id="item_id" class="form-control itemOptions single-select req">
							...
							<table class="table table-bordered">
								<thead class="thead-info">
									<tr>
										...
										<th class="text-center" style="width:15%;">Grn. No.</th>
										<th class="text-center" style="width:20%;">Challan No.</th>
										<th class="text-center" style="width:10%;">Grn. Date</th>
...
<div class="modal fade" id="qcPurchaseModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel1" data-backdrop="static" data-keyboard="false">
	<div class="modal-dialog modal-lg" role="document" style="min-width:70%;">
		<div class="modal-content animated slideDown">
			<div class="modal-header">
				<h4 class="modal-title" id="createInvoice">Qc Purchase List</h4>
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			</div>
			<form id="party_qc" method="post" action="">
				<div class="modal-body scrollable" style="height:60vh;">
					<input type="hidden" name="party_id" id="party_id_pinv" value="">
					<input type="hidden" name="party_name" id="party_name_pinv" value="">
					<div class="col-md-12">
						...
						<div class="table-responsive">
							<table class="table table-bordered">
								<thead class="thead-info">
									<tr>
										<th class="text-center" style="width:5%;">#</th>
										<th class="text-center" style="width:15%;">Ord. No.</th>
										<th class="text-center" style="width:10%;">Ord. Date</th>
										<th class="text-left">Item Name</th>
...
if (!empty($orderItems)) :
	...
	foreach ($orderItems as $row) :
		...
		$row->stateCode = "";
		$row->net_amount = (($row->amount - $row->disc_amt) + $row->igst_amt);
		echo '<script>AddRow(' . json_encode($row) . ');</script>';
	endforeach;

endif;

if(!empty($qcOrderItems)):
	$i = 1;
	foreach ($qcOrderItems as $row) :
		$row->row_index = "";
		$row->trans_id = "";
		$row->from_entry_type = -1;
		$row->item_name = "[" . $row->item_code . "] " . $row->item_name;
		$row->ref_id = $row->id;
		$row->qty = round(($row->rec_qty - $row->inv_qty), 2);
		$row->qty_kg = "0.000";

		$row->cgst_per = $row->cgst;
		$row->sgst_per = $row->sgst;
		$row->igst_per = $row->igst;
		$row->cgst_amt = $row->cgst_amt;
		$row->sgst_amt = $row->sgst_amt;
		$row->igst_amt = $row->igst_amt;
