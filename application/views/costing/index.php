                ...
                <div class="card">
                    ...
                    <div class="card-body"> 
                        <div class="table-responsive">
                            <table id='costingTable' class="table table-bordered ssTable" data-url='/getDTRows'></table>
                        </div>
                    </div>
                </div>
            </div>
...
$(document).ready(function() {
    $(document).on('click', ".addCosting", function() {
        var id = $(this).data('id');
        ...
        $.ajax({
            type: "POST",
            url: base_url + 'costing/' + functionName,
            data: {id: id}
        }).done(function(response) {
            $("#" + modalId).modal();
            $("#" + modalId + " .modal-dialog").css('max-width','50%');
            $("#" + modalId + ' .modal-title').html(title + " [ "+productName+" ]");
