...
class QcChallanModel extends MasterModel{
    ...
    public function getDTRows($data){
        ...
        $data['select'] = 'qc_challan_trans.*,qc_challan.trans_prefix,qc_challan.trans_no,qc_challan.challan_type,qc_challan.trans_date,qc_challan.party_id,qc_instruments.item_name,qc_instruments.item_code,(CASE WHEN qc_challan.challan_type != 1 THEN party_master.party_name ELSE department_master.name END) as party_name,qc_challan.remark';
        ...
        if(empty($data['qc_status'])){
            $data['where']['qc_challan_trans.trans_status'] = 0;    
        }else{ 
		    $data['where']['qc_challan_trans.trans_status >'] = 0; 
			$data['where']['qc_challan.trans_date >= '] = $this->startYearDate;
			$data['where']['qc_challan.trans_date <= '] = $this->endYearDate;
		}
        ...
        $data['order_by']['qc_challan.trans_date'] = 'DESC';   
        ...
        $data['searchCol'][] = "qc_challan.remark";

		$columns =array('','','qc_challan.trans_no','qc_challan.trans_date','qc_challan.challan_type','qc_instruments.item_code','party_name','qc_instruments.item_name','qc_challan.remark');
	...
	public function save($masterData,$itemData){
        try{
            $this->db->trans_begin();
            $item_type = $masterData['item_type']; unset($masterData['item_type']);
            
            if(empty($masterData['id'])):
                $masterData['trans_no'] = $this->nextTransNo(1);
                $inChallan = $this->store($this->qcChallan,$masterData);
                $mainId = $inChallan['insert_id'];
                if($item_type == 1):
                    $result = ['status'=>1,'message'=>'Challan updated Successfully.','url'=>base_url("qcGauges")];
                else:
                    $result = ['status'=>1,'message'=>'Challan updated Successfully.','url'=>base_url("qcInstrument")];
                endif;
            else:
            ...
            foreach($itemData['item_id'] as $key=>$value):
                $item = $this->qcInstrument->getItem($value);
                $transData = [
                    'id' => $itemData['id'][$key],
                    'challan_id' => $mainId,
                    'item_id' => $value,
	...
	public function getQcChallan($id){
        ...
        $queryData['select'] = 'qc_challan.*,(CASE WHEN qc_challan.challan_type != 1 THEN party_master.party_name ELSE department_master.name END) as party_name';
    ...
    public function getQcChallanTrans($id){
        ...
        $queryData['select'] = 'qc_challan_trans.*,qc_instruments.item_name,qc_instruments.item_code,qc_instruments.make_brand,qc_instruments.size,qc_instruments.cal_freq';
        $queryData['join']['qc_instruments'] = "qc_challan_trans.item_id = qc_instruments.id";
        $queryData['where']['challan_id'] = $id;
        return $this->rows($queryData);
    }
    
	// Updated By Meghavi @12/01/2024
    public function getQcChallanTransRow($id){
        $queryData = array();
        $queryData['tableName'] = $this->qcChallanTrans;
        $queryData['select'] = 'qc_challan_trans.*,qc_challan.party_id,qc_instruments.item_name,qc_instruments.item_code,qc_instruments.size_go,qc_instruments.size_nogo,qc_instruments.ac_go,qc_instruments.ac_nogo,party_master.party_name';
    ...
    public function confirmChallan($data){
        try{
            $this->db->trans_begin();
                ...
                $this->edit($this->qcChallanTrans, ['id'=>$data['id']], ['trans_status'=>1]);
                ...
                $result = ['status'=>1,'message'=>'Challan Confirm Successfully.'];
            if ($this->db->trans_status() !== FALSE):
                $this->db->trans_commit();
                return $result;
            endif;
