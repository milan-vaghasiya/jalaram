...
class AttendanceModel extends MasterModel
{
	...
	public function getAttendanceStatsByDate($filterDate)
	{
		...
		if (!empty($totalEmp)) {
			$presentPer = round((($present * 100) / $totalEmp), 2);
		}
		if (!empty($totalEmp)) {
			$absentPer = round((($absent * 100) / $totalEmp), 2);
		}
		if (!empty($present)) {
			$latePer = round((($late * 100) / $present), 2);
		}
		...
		foreach ($empData as $emp) :
			...
			if (!empty($missedPunch)) :
				...
				if (!empty($punchTimes)) :
					...
					if ($dorn == 2) {
						$emp->punch_time = implode(', ', sortDates($punchTimes, 'DESC'));
					}
				endif;
	...
	public function getExtraHours($filterDate, $emp_id = "")
	{
		...
		$mpQuery['where']['punch_type'] = 3;
		$mpQuery['where']['punch_date'] = $filterDate;
		$mpQuery['tableName'] = 'attendance_log';
	...
	public function loadAttendanceSheet($month)
	{
		...
		if (!empty($empData)) {
			foreach ($empData as $emp) {
				...
				if (!empty($emp->emp_profile)) {
					$profile_pic = base_url('assets/images/users/' . $emp->emp_profile);
				}
		...
		if (!empty($empData)) {
			foreach ($empData as $emp) {
				...
				for ($d = $first_day; $d <= $last_day; $d++) {
					...
					if (empty($overtime) or $overtime < 0) {
						$overtime = 0;
					} else {
						$overtime = $overtime . ':' . $total_is . ' Hrs';
					}
					$attendanceDate = date("d-m-Y", strtotime($year . '-' . $month . '-' . $d));
					$infotitle = date("jS, F Y ", strtotime($year . '-' . $month . '-' . $d));
					$totalhour = $total_hours . ' Hrs';
	...
	public function saveBiometricData($fdate, $tdate, $Empcode = "ALL")
	{
		...
		if ($err) {
			...
			$resultapi = json_decode($response);
			$punchData = $resultapi->PunchData;
			$cfdate = date("Y-m-d 00:00:01");
			$ctdate = date("Y-m-t 23:59:59");
	...
	/*public function getEmployeeListOLD($biomatric_id = '',$shift_start='',$shift_end='')
	{
		$empQuery['select'] = "employee_master.*,department_master.name, department_master.section, emp_designation.title, emp_category.category,attendance_policy.early_in as late_in, attendance_policy.early_out,attendance_policy.no_early_in as no_late_in,attendance_policy.no_early_out,shift_master.start_time as shift_start,shift_master.end_time as shift_end,shift_master.shift_name,shift_master.shift_hour as total_shift_time";
		$empQuery['leftJoin']['emp_designation'] = "emp_designation.id = employee_master.emp_designation";
		$empQuery['leftJoin']['department_master'] = "employee_master.emp_dept_id = department_master.id";
		$empQuery['leftJoin']['emp_category'] = "employee_master.emp_category = emp_category.id";
		$empQuery['leftJoin']['attendance_policy'] = "attendance_policy.id = employee_master.attendance_policy";
		...
		$empQuery['where']['employee_master.is_active'] = 1;
		$empQuery['where']['employee_master.attendance_status'] = 1;
		$empQuery['order_by']['employee_master.emp_code'] = 'ASC';
		$empQuery['tableName'] = $this->empMaster;
		$empData = $this->rows($empQuery);
		// print_r($empData);
	...
	public function getEmployeeList($postData = Array())
	{
		$empQuery['select'] = "employee_master.*,department_master.name, department_master.section, emp_designation.title, emp_category.category,attendance_policy.early_in as late_in, attendance_policy.early_out,attendance_policy.no_early_in as no_late_in,attendance_policy.no_early_out,shift_master.start_time as shift_start,shift_master.end_time as shift_end,shift_master.shift_name,shift_master.shift_hour as total_shift_time";
		$empQuery['leftJoin']['emp_designation'] = "emp_designation.id = employee_master.emp_designation";
		$empQuery['leftJoin']['department_master'] = "employee_master.emp_dept_id = department_master.id";
		...
		if (!empty($postData['dept_id'])) {$empQuery['where']['employee_master.emp_dept_id'] = $postData['dept_id'];}
	...
	public function getAttendanceLogByEmp($punchDate, $empId)
	{
		...
		$queryData['customWhere'][] = 'punch_date BETWEEN "' . date('Y-m-d H:i:s', strtotime($punchDate . ' 00:00:01')) . '" AND "' . date('Y-m-d H:i:s', strtotime($punchDate . ' 23:59:59')) . '"';
		$queryData['where']['emp_id'] = $empId;
		$queryData['where_in']['punch_type'] = "1,2";
		return $this->row($queryData);
	}

	public function getEmpPunches($punchDate, $empId)
	{
		$toDate = date('Y-m-d', strtotime($punchDate . ' +1 day'));
		$queryData['tableName'] = 'attendance_log';
		$queryData['select'] = 'punch_date,punch_type';
	...
	public function getEmpMonthlyPunches($month, $empId)
	{
		$queryData['tableName'] = 'attendance_log';
		$queryData['select'] = ' GROUP_CONCAT(punch_date) as punch_date,DAY(punch_date) AS day';
		$queryData['where']['emp_id'] = $empId;
		$queryData['where']['MONTH(punch_date)'] = $month;
		$queryData['where_in']['punch_type'] = "1,2";
		$queryData['group_by'][] = ' DAY(punch_date)';
		$result = $this->rows($queryData);
		return $result;
	}
	public function getEmpPunchesCount($punchDate, $empId)
	{
		$toDate = date('Y-m-d', strtotime($punchDate . ' +1 day'));
		$queryData['tableName'] = 'attendance_log';
		// $queryData['select'] = 'count(*)';
		$queryData['customWhere'][] = 'punch_date BETWEEN "' . date('Y-m-d H:i:s', strtotime($punchDate . ' 00:00:01')) . '" AND "' . date('Y-m-d H:i:s', strtotime($toDate . ' 23:59:59')) . '"';
		$queryData['where']['emp_id'] = $empId;
		$queryData['where_in']['punch_type'] = "1,2";
	...
	public function getEmpLeaveMonthWise($filterDate, $emp_id)
	{
		$leaveQuery['where']['emp_id'] = $emp_id;
		$leaveQuery['customWhere'][] = '"' . $filterDate . '" BETWEEN start_date AND end_date';
		$leaveQuery['where']['approve_status'] = 1;
		$leaveQuery['tableName'] = $this->leaveMaster;
		$leaveData = $this->row($leaveQuery);
	...
	public function getHolidaysMonthWise($month)
	{
        $data['tableName'] = 'holidays';
		$data['select']='count(*) as total_holidays';
		$data['where']['MONTH(holiday_date)'] = $month;
        return $this->row($data);
	}

	public function getAttendanceShiftLogData($attendance_date,$emp_id)
	{
		$data['tableName'] = 'emp_shift_log';
		$data['select']='emp_shift_log.*';
		$data['where']['emp_id'] = $emp_id;
		$data['where']['attendance_date'] = $attendance_date;
        return $this->row($data);
	}
	public function getEmpPunchesDateWise($shift_start,$shift_end,$empId='')
	{
		$queryData['tableName'] = 'attendance_log';
		$queryData['select'] = 'punch_date';
		$queryData['customWhere'][] = 'punch_date BETWEEN "' . date('Y-m-d H:i:s', strtotime($shift_start)) . '" AND "' . date('Y-m-d H:i:s', strtotime($shift_end)) . '"';
		if(!empty($empId)){$queryData['where']['emp_id'] = $empId;}
		$queryData['where_in']['punch_type'] = "1,2";
		return $this->rows($queryData);
	}
	
    /*** Created By : JP@21.10.2022***/
	public function getEmpPunchesByDate($empId,$punchDate)
	{
		$toDate = date('Y-m-d', strtotime($punchDate . ' +1 day'));
		
		$queryData['tableName'] = 'employee_master';
		$queryData['select'] = "employee_master.emp_code,attendance_log.punch_date,attendance_log.punch_type,
		CAST(DATE_SUB(CONCAT(emp_shift_log.attendance_date, ' ', emp_shift_log.shift_start), INTERVAL 2 HOUR) as datetime) as shiftStart,
		(CASE WHEN emp_shift_log.shift_type=1 THEN CAST(CONCAT(emp_shift_log.attendance_date, ' ', emp_shift_log.shift_end) as datetime) ELSE 
		CAST(CONCAT(DATE_ADD(emp_shift_log.attendance_date, INTERVAL 22 HOUR), ' ', emp_shift_log.shift_end) as datetime) END) as shiftEnd,
		CAST(CONCAT(DATE_ADD(emp_shift_log.attendance_date, INTERVAL 22 HOUR), ' ', emp_shift_log.shift_start) as datetime) as endTime24";
		$queryData['leftJoin']['attendance_log'] = "attendance_log.emp_id = employee_master.id";
		$queryData['leftJoin']['emp_shift_log'] = "emp_shift_log.emp_id = employee_master.id";
		$queryData['where']['emp_shift_log.attendance_date'] = date('Y-m-d', strtotime($punchDate));
		$queryData['where']['employee_master.id'] = $empId;
		$queryData['where_in']['attendance_log.punch_type'] = "1,2";
		$queryData['having'][] = "attendance_log.punch_date >= shiftStart AND attendance_log.punch_date < endTime24";
		$result = $this->rows($queryData);
