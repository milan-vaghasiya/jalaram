...
class PurchaseInvoiceModel extends MasterModel{
	...
	public function getPartyOrders($id){
        ...
        $queryData['where']['grn_date >= '] = $this->startYearDate;
        $queryData['where']['grn_date <= '] = $this->endYearDate;
	...
	public function getGrnItemsForInvoice($grnIds){
        ...
        $data['select'] = "grn_transaction.*,item_master.item_name,item_master.item_code,unit_master.unit_name,item_master.item_type, item_master.gst_per,purchase_order_trans.disc_per";
		...
		try{
			...
			if($checkDuplicate > 0):
                ...
                if(empty($masterData['id'])):
			        $masterData['trans_prefix'] = $this->transModel->getTransPrefix(12);
                    $masterData['trans_no'] = $this->transModel->nextTransNo(12);
                    $masterData['trans_number'] = $this->getPrefixNumber($masterData['trans_prefix'],$masterData['trans_no']);
                endif;

                if(!empty($purchaseId)):
                    $invoiceItems = $this->purchaseTransactions($purchaseId);

                    foreach($invoiceItems as $row):
                        if(!empty($row->ref_id) && $row->from_entry_type != -1):
                            $this->store($this->grnTrans,['id'=>$row->ref_id,'trans_status'=>0]);
                        endif;
                    ...
                    endforeach;
					$this->trash($this->transChild,['trans_main_id'=>$purchaseId]);
				endif;
				...
				foreach($itemData['item_id'] as $key=>$value):
                    ...
                    if(!empty($itemData['ref_id'][$key]) && $itemData['from_entry_type'][$key] != -1):
                        $this->store($this->grnTrans,['id'=>$itemData['ref_id'][$key],'trans_status'=>1]);
                    endif;

                    if(!empty($itemData['ref_id'][$key]) && $itemData['from_entry_type'][$key] == -1): //Qc Order
                        $setData = Array();
                        $setData['tableName'] = $this->purchaseOrderTrans;
                        $setData['where']['id'] = $itemData['ref_id'][$key];
                        $setData['set']['inv_qty'] = 'inv_qty, + '.$itemData['qty'][$key];
                        $this->setValue($setData);
                    endif;
                endforeach;
                ...
                if(!empty($masterData['ref_id'])):
                    $pendingGrn = $this->checkGrnPendingStatus($masterData['ref_id']);
                    if(empty($pendingGrn)) :
                        $this->store($this->grnMain, ['id' => $masterData['ref_id'],'trans_status' => 1]);
                    endif;
                endif;
				$result = ['status'=>1,'message'=>'Purchase Invoice saved successfully.','url'=>base_url("purchaseInvoice")];	
			endif;

            /* Send Notification */
            $piNo = $masterData['doc_no'];
            $notifyData['notificationTitle'] = (empty($masterData['id']))?"New Purchase Invoice":"Update Purchase Invoice";
            $notifyData['notificationMsg'] = (empty($masterData['id']))?"New Purchase Invoice Generated. Inv. No. : ".$piNo:"Purchase Invoice updated. Inv No. : ".$piNo;
            $notifyData['payload'] = ['callBack' => base_url('purchaseInvoice')];
            $notifyData['controller'] = "'purchaseInvoice'";
    ...
    public function delete($id){
		try{
            ...
            endforeach;
            if(!empty($invoiceData->ref_id)):
                $pendingGrn = $this->checkGrnPendingStatus($invoiceData->ref_id);
                if(!empty($pendingGrn)):
                    $this->store($this->grnMain, ['id' => $invoiceData->ref_id,'trans_status' => 0]);
                endif;
            endif;
            ...
            $this->transModel->deleteExpenseTrans($id);
                ...
                /* Send Notification */
                $piNo = $invoiceData->doc_no;
                $notifyData['notificationTitle'] = "Delete Purchase Invoice";
                $notifyData['notificationMsg'] = "Purchase Invoice deleted. Inv No. : ".$piNo;
                $notifyData['payload'] = ['callBack' => base_url('purchaseInvoice')];
                $notifyData['controller'] = "'purchaseInvoice'";
                $notifyData['action'] = "D";
                $this->notify($notifyData);
