...
class ShiftModel extends MasterModel{
    ...
    public function saveManageShift($data){
		try {
            $this->db->trans_begin();
			$result = Array();
            if(!empty($data['id']))
			{
				...
				$result = $this->edit($this->empShiftLog, $where, [$data['field_id'] => $data['new_shift_id'], 'updated_by' => $data['updated_by'], 'updated_at' => $data['updated_at']]);
				
				// Update Employee Shift in Master data if Manage Shift Date is Today
				if(!empty($data['new_shift_id']))
				{
    				$shift_date = date('Y-m-d',strtotime($data['shift_date']));
    				if(($result['status']==1) AND $shift_date==date('Y-m-d'))
    				{
    				    $shiftData = $this->getShiftByLatestId($data['new_shift_id']);
    				    if(!empty($shiftData->id))
    				    {
                		    $updatempShift = $this->store('employee_master',['id' => $data['emp_id'],'shift_id' => $shiftData->id]);
    				    }
    				}
				}
			}
